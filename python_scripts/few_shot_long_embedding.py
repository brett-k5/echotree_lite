...
if __name__ == "__main__":
    file_path = os.path.join(os.path.dirname(__file__), "..", "few-shot-long.txt")
    with open(file_path, "r", encoding="utf-8") as f:
        few_shot_long = f.read()

    load_dotenv()
    gemini_api_key = os.getenv("GEMINI_API_KEY")
    client = genai.Client(api_key=gemini_api_key)

    text_chunks = split_few_shots(few_shot_long)
    ...
    out_path = os.path.join(os.path.dirname(__file__), "few_shot_long_embeddings.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(embeddings, f, ensure_ascii=False, indent=2)

    print(f"Saved {len(embeddings)} embeddings to {out_path}")

    out_path = os.path.join(os.path.dirname(__file__), "few_shot_long_chunks.json")
